...
char* getWordPtr(pointerToTrieNode node){
    return (node->wordptr == NULL ? node->word : node->wordptr);
}
...
{/*Inserts a new child in the node father*/
    ...
    if(whereToInsert>=0)
    {
        ...
        if(strlen(wordToInsert) < SIZE){
        	...
        	father->children[whereToInsert].wordptr=NULL;
        }
        else{
...
int searchPositionBinary(pointerToTrieNode children, int numofchildren, int actualnumofchildren, char* wordToInsert, int *exactPos)
{
    ...
    while(lowerBound<=upperBound)
    {
        ...
        if(children[midPoint].wordptr != NULL) pointerToWord = children[midPoint].wordptr;
        else pointerToWord = children[midPoint].word;

        if(strcmp(pointerToWord ,wordToInsert)<0)
        {
            lowerBound=midPoint+1;
        }
        else if(strcmp(pointerToWord ,wordToInsert)>0)
        {
...
void printTrieNode(pointerToTrieNode nodeToPrint)
{
    ...
    else if(strcmp(nodeToPrint->word, "") != 0){
        printf("Word:%s\n", nodeToPrint->word);
    }
    else
    {
    ...
    if(nodeToPrint->actualNumOfChildren>0)
    {
        ...
        for(i=0;i<nodeToPrint->actualNumOfChildren;++i)
        {
            ...
            if(nodeToPrint->children[i].wordptr != NULL) printf("Child's word:%s\n",nodeToPrint->children[i].wordptr);
            else printf("Child's word:%s\n",nodeToPrint->children[i].word);
            printf("\n");
        }
    }
...
void destroyNode(pointerToTrieNode node){
    ...
    if(node->wordptr != NULL) free(node->wordptr); 
    free(node);
}
...
void deleteANode(pointerToTrieNode father, int childToDelete)
{
    ...
    if(father->children[childToDelete].wordptr != NULL){
