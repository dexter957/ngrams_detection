...
typedef struct static_trie_node{
    ...
    short lengths[LENGTHS], *lengthsptr, numOfWords;
...
int static_getIs_FinalPtr(pointerToStaticTrieNode node){
    return (node->lengthsptr[node->numOfWords-1] > 0 ? TRUE : FALSE);
}

int static_getIs_FinalPositionPtr(pointerToStaticTrieNode node, int position){
    return (node->lengthsptr[position] > 0 ? TRUE : FALSE);
}
...
char* static_getWordPositionPtr(pointerToStaticTrieNode node, int position){
    ...
    if(position < node->numOfWords){
    	len = getAbsoluteValue(node->lengthsptr[position]);
    	for(i = 0; i < position; i++){
    		offset += node->lengthsptr[i];
    	}
...
void static_setIs_FinalPtr(pointerToStaticTrieNode node, int finality){
    if(finality == TRUE) node->lengthsptr[node->numOfWords-1] = strlen(node->wordptr);
    else node->lengthsptr[node->numOfWords-1] = (short)((strlen(node->wordptr) ^ OPPOSITE_MASK) + 1);
...
void static_setWordPtr(pointerToStaticTrieNode node, char* theWord){
    ...
    if(node->lengthsptr != NULL && node->lengthsptr != node->lengths) free(node->lengthsptr);
    node->lengthsptr = node->lengths;
...
{/*Inserts a new child in the node father*/
    ...
    if(whereToInsert>=0)
    {
        ...
        father->children[whereToInsert].lengthsptr = father->children[whereToInsert].lengths;
...
int searchStatic(pointerToStaticTrieNode node, char *wordToInsert, int numOfVisits){
	...
	if(numOfVisits <= node->numOfWords-1){
		if(getAbsoluteValue(node->lengthsptr[numOfVisits]) == strlen(wordToInsert)){
			for(i = 0; i < numOfVisits; i++){
				offset += getAbsoluteValue(node->lengthsptr[i]);
			}
			if(strncmp(node->wordptr+offset, wordToInsert, getAbsoluteValue(node->lengthsptr[numOfVisits])) == 0){
				return -2;
			}
            else{
                return -1;
...
int searchPositionBinary2(pointerToStaticTrieNode children, int numofchildren, int actualnumofchildren, char* wordToInsert)
{
    ...
    {
        midPoint=(upperBound+lowerBound)/2;
        test_len = getAbsoluteValue(children[midPoint].lengthsptr[0]);
...
void static_printTrieNode(pointerToStaticTrieNode nodeToPrint)
{
    ...
    if(nodeToPrint->wordptr!=NULL)
    {
        if(nodeToPrint->numOfWords == 1) printf("Word:%s\n", nodeToPrint->wordptr);
        	...
        	for(i = 0; i < nodeToPrint->numOfWords; i++){
                if(nodeToPrint->lengthsptr[i] < 0) length = ((nodeToPrint->lengthsptr[i] ^ OPPOSITE_MASK) + 1); 
        		else length = nodeToPrint->lengthsptr[i];
    ...
    else
    {
     ...
     printf("Final node:");
    if(nodeToPrint->lengthsptr==NULL)
    {
    	printf("unknown\n");
    }
    else
    {
        for(i = 0; i < nodeToPrint->numOfWords; i++){
        	if(nodeToPrint->lengthsptr[i] < 0) printf("FALSE ");
        	else printf("TRUE ");
        }
    }
    printf("\n");
...
void static_destroyNode(pointerToStaticTrieNode node){
	...
	if(node->lengthsptr != node->lengths) free(node->lengthsptr);
	free(node);
}
...
void static_deleteANode(pointerToStaticTrieNode father, int childToDelete)
{
    ...
    if(father->children[childToDelete].lengthsptr != NULL && father->children[childToDelete].lengthsptr != father->children[childToDelete].lengths){
    	free(father->children[childToDelete].lengthsptr);
    }
...
void static_repointToWords(pointerToStaticTrieNode children, int size){
	...
	for(i = 0; i < size; i++){
        ...
        if(children[i].numOfWords <= LENGTHS){
            children[i].lengthsptr = children[i].lengths;
        }
	}
}
		...
		if(inserted == FALSE){
			...
			if(current->actualNumOfChildren > positions[i]){
				...
				if(current->actualNumOfChildren == 1){
                    ...
                    if(temp_numOfWords > LENGTHS){
                        temp_lengths = malloc(sizeof(short)*temp_numOfWords);
                        memmove(temp_lengths, current->lengthsptr, sizeof(short)*current->numOfWords);
                        memmove(temp_lengths+current->numOfWords, child->lengthsptr, sizeof(short)*child->numOfWords);
                        if(current->lengthsptr != current->lengths) free(current->lengthsptr);
                        current->lengthsptr = temp_lengths;
                    }
                    else{
                        memmove(current->lengthsptr+current->numOfWords, child->lengthsptr, sizeof(short)*child->numOfWords);
                    }
