...
void static_setIs_FinalPtr(pointerToStaticTrieNode node, int finality){
    ...
    else node->lengthsptr[node->numOfWords-1] = strlen(node->wordptr) * (-1);
}
    ...
    if(nodeToPrint->wordptr!=NULL)
    {
        if(nodeToPrint->numOfWords == 1) printf("Word:%s\n", nodeToPrint->wordptr);
        	...
        	for(i = 0; i < nodeToPrint->numOfWords; i++){
        		if(nodeToPrint->lengthsptr[i] < 0) length = nodeToPrint->lengthsptr[i] * (-1); 
...
void shrink(pointerToStaticTrieNode node){
	...
	if(node->actualNumOfChildren == 1){
        ...
        if(temp_numOfWords > LENGTHS){
    		...
    		memmove(temp_lengths, node->lengthsptr, sizeof(short)*node->numOfWords);
    		memmove(temp_lengths+node->numOfWords, child->lengthsptr, sizeof(short)*child->numOfWords);
    		if(node->lengthsptr != node->lengths) free(node->lengthsptr);
    		node->lengthsptr = temp_lengths;
        }
        else{
            memmove(node->lengthsptr+node->numOfWords, child->lengthsptr, sizeof(short)*child->numOfWords);
        }
